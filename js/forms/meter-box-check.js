(function () {
  const FORM_ID = 'meter-box-check';
  const FORM_TITLE = '售電表箱檢查表';
  const FILE_PREFIX = '1-';

  const CHECK_ITEMS = [
    { no: '9.1', label: '9.1 確認電錶是否有正常運行' },
    { no: '9.2', label: '9.2 紀錄電錶顯示序號109之讀值' },
  ];
  ...
  els.bulkExportBtn.addEventListener('click', async (e) => {
    ...
    try {
      ...
      for (const id of ids) {
        ...
        if (!rec) continue;
        pages.push(MeterBoxCheckTemplate.buildPage1(rec.data, bg));
  ...
  function validateForExport() {
    ...
    if (photoCtrls.meterReading.getPhotos().length === 0) missing.push('「檢查項目-電錶顯示序號109之讀值」尚未上傳照片');
    if (photoCtrls.meterBox.getPhotos().length === 0) missing.push('「檢查項目-躉售表箱」尚未上傳照片');
    return missing;
  }
  ...
  const TEMPLATE_BG_PATH = '../pdf-templates/meter-box-check-p1.png';
  ...
  async function buildCurrentPdfBlob() {
    ...
    const data = gatherFormData();
    const bg = await loadBackground();
    const page = MeterBoxCheckTemplate.buildPage1(data, bg);
