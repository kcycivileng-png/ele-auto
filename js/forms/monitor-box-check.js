(function () {
  const FORM_ID = 'monitor-box-check';
  const FORM_TITLE = '發電廠監控箱檢查表';
  const FILE_PREFIX = '1-';

  const CHECK_ITEMS = [
    { no: '5.1', label: '5.1 日照計檢查及清潔' },
    { no: '5.2', label: '5.2 日照計安裝是否穩固' },
    { no: '5.3', label: '5.3 日照計是否進行誤差值比對(該年度誤差值比對日期：)' },
    { no: '5.4', label: '5.4 模組溫度計是否有貼妥' },
    { no: '5.5', label: '5.5 錶頭是否正常顯示讀值' },
    { no: '5.6', label: '5.6 錶頭是否有滲水、起霧現象' },
    { no: '5.7', label: '5.7 箱體外觀是否銹蝕、破損、變形' },
    { no: '5.8', label: '5.8 箱體內是否有滲水現象、防水膠條是否完善' },
    { no: '5.9', label: '5.9 箱體內部是否有異物並做清潔' },
    { no: '5.10', label: '5.10 箱體是否有動物或動物侵入痕跡' },
    { no: '5.11', label: '5.11 監控主機是否運作正常' },
    { no: '5.12', label: '5.12 線路是否固定、無脫落' },
    { no: '5.13', label: '5.13 檢查各線路、鎖固接點是否鬆脫' },
    { no: '5.14', label: '5.14 檢查是否有異味、各電器是否有燒焦痕跡或其它異常現象' },
  ];
...
max: g.max,
  ...
  els.bulkExportBtn.addEventListener('click', async (e) => {
    ...
    try {
      ...
      for (const id of ids) {
        ...
        if (!rec) continue;
        pages.push(MonitorBoxCheckTemplate.buildPage1(rec.data, bg.p1));
        pages.push(MonitorBoxCheckTemplate.buildPagePhotos(rec.data, bg.p2));
  ...
  function validateForExport() {
    ...
    if (photoCtrls.sunSensor.getPhotos().length === 0) missing.push('「檢查項目-日照計」尚未上傳照片');
    if (photoCtrls.monitorBox.getPhotos().length === 0) missing.push('「檢查項目-監控箱」尚未上傳照片');
    return missing;
  }
  ...
  const TEMPLATE_BG_PATHS = {
    p1: '../pdf-templates/monitor-box-check-p1.png',
    p2: '../pdf-templates/monitor-box-check-p2-photos.png',
  };
  ...
  async function buildCurrentPdfBlob() {
    ...
    const data = gatherFormData();
    const bg = await loadBackgrounds();
    const pages = [MonitorBoxCheckTemplate.buildPage1(data, bg.p1), MonitorBoxCheckTemplate.buildPagePhotos(data, bg.p2)];
    const blob = await KtPdf.renderTemplatedPdf(pages);
