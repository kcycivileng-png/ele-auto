(function () {
  const FORM_ID = 'dcbox-check';
  const FORM_TITLE = '直流箱檢查表';
  const FILE_PREFIX = '1-';
  const TABLE_ROWS = 10; // 比照原始Word表格，固定10列

  const CHECK_ITEMS = [
    { no: '2.1', label: '2.1 箱體外觀是否銹蝕、破損、變形' },
    { no: '2.2', label: '2.2 箱體內是否有滲水現象、防水膠條是否完善' },
    { no: '2.3', label: '2.3 檢查箱體內部是否有異物並做清潔' },
    { no: '2.4', label: '2.4 檢查是否有動物或動物侵入痕跡' },
    { no: '2.5', label: '2.5 檢查是否有異味、各電器是否有燒焦痕跡或燒毀現象' },
    { no: '2.6', label: '2.6 檢查突波吸收器是否正常' },
    { no: '2.7', label: '2.7 檢查各接點是否鬆脫' },
    { no: '2.8', label: '2.8 箱內各零組件、接點，溫度是否正常' },
    { no: '2.9', label: '2.9 外部纜線是否損傷、接續端子是否鬆脫' },
    { no: '2.10', label: '2.10 箱體可否正常開啟及關閉' },
    { no: '2.11', label: '2.11 箱體正面是否正確張貼警示警語' },
    { no: '2.12', label: '2.12 箱內是否放置正確電路圖' },
    { no: '2.13', label: '2.13 保險絲是否正常無熔斷' },
  ];

  // 對應原始表格「DC BOX熱顯像」3組設備框格（設備1一定要有，2/3視電廠實際台數選填）
  // 及「異常項目」2格（熱顯像/絕緣/接地各自的異常證據照，無異常則不需提供）。
  const THERMAL_PHOTO_GROUPS = [
    { id: 'thermalIr1', title: '檢查項目-DC BOX熱顯像儀 設備1 (熱顯像相片)', single: true },
    { id: 'thermalVis1', title: '檢查項目-DC BOX熱顯像儀 設備1 (可見光對照相片)', single: true },
    { id: 'thermalIr2', title: '檢查項目-DC BOX熱顯像儀 設備2 (熱顯像相片，選填)', single: true },
    ...
    { id: 'thermalAbn1', title: '檢查項目-熱顯像異常項目1 (異常時提供，無則免填)', single: true },
    { id: 'thermalAbn2', title: '檢查項目-熱顯像異常項目2 (異常時提供，無則免填)', single: true },
  ];
  ...
  // ---- 電廠名稱：群組→館別兩階下拉選單（與太陽能模組表共用同一份場站清單） ----
  ...
  // ---- 建立組串絕緣電阻／接地電阻量測表（各固定10列，比照Word） ----
  const insulationTable = KtFormKit.createEditableTable(document.getElementById('insulationTableWrap'), {
    fixedRows: TABLE_ROWS,
    onChange: scheduleAutosave,
    columns: [
      { key: 'circuit', label: '迴路編號', type: 'text', placeholder: '1-1' },
  ...
  const groundingTable = KtFormKit.createEditableTable(document.getElementById('groundingTableWrap'), {
    ...
    onChange: scheduleAutosave,
    columns: [
      { key: 'boxNo', label: '直流箱編號', type: 'text', placeholder: '1' },
  ...
  function blankFormData() {
    return {
      ...
      photoGroups: ALL_PHOTO_GROUPS.map((g) => ({ id: g.id, title: g.title, photos: [] })),
    };
  }
  ...
  // 複製紀錄：只複製文字/勾選欄位，不複製照片——避免誤用到別次的現場照片。
  async function duplicateRecord(rec) {
    ...
    const copied = {
      ...
      insulationRows: (src.insulationRows || []).map((r) => ({ ...r })),
  ...
  function gatherFormData() {
    return {
      ...
      insulationRows: insulationTable.getRows(),
      ...
      photoGroups: ALL_PHOTO_GROUPS.map((g) => ({ id: g.id, title: g.title, photos: photoCtrls[g.id].getPhotos() })),
    };
  }
  ...
  function applyFormData(data) {
    ...
    insulationTable.setRows(data.insulationRows || []);
  ...
  function validateForExport() {
    ...
    if (photoCtrls.thermalIr1.getPhotos().length === 0) missing.push('「DC BOX熱顯像儀 設備1 (熱顯像相片)」尚未上傳照片');
    if (photoCtrls.thermalVis1.getPhotos().length === 0) missing.push('「DC BOX熱顯像儀 設備1 (可見光對照相片)」尚未上傳照片');

    const insulationRows = insulationTable.getRows();
    const groundingRows = groundingTable.getRows();
    const hasAbnormalInsulation = insulationRows.some((r) => r.result === '不合格');
    ...
    if (hasAbnormalInsulation && photoCtrls.insulAbn1.getPhotos().length === 0 && photoCtrls.insulAbn2.getPhotos().length === 0) {
      missing.push('組串絕緣電阻量測有標示不合格，請提供「組串絕緣電阻異常」照片');
    }
  ...
  // 底圖只需要載入一次，快取起來給每次匯出重複使用（對應原始文件P20~P23）
  const TEMPLATE_BG_PATHS = {
    p1: '../pdf-templates/dcbox-check-p1.png',
    p2: '../pdf-templates/dcbox-check-p2-thermal.png',
    p3: '../pdf-templates/dcbox-check-p3-insulation.png',
    p4: '../pdf-templates/dcbox-check-p4-grounding.png',
  };
  ...
  async function buildCurrentPdfBlob() {
    ...
    const bg = await loadBackgrounds();
    const T = DcboxCheckTemplate;
    // 只輸出Word原始頁面（P20~P23），一律輸出全部4頁（原文件「無異常則不刪減下方表格」）。
    const pages = [
  ...
  const CSV_HEADERS = [
    ...
    '絕緣電阻量測日期',
    ...Array.from({ length: TABLE_ROWS }, (_, i) => [`絕緣${i + 1}迴路編號`, `絕緣${i + 1}量測值(MΩ)`, `絕緣${i + 1}結果`, `絕緣${i + 1}改善對策`]).flat(),
    '接地電阻量測日期',
    ...Array.from({ length: TABLE_ROWS }, (_, i) => [`接地${i + 1}箱編號`, `接地${i + 1}量測值(Ω)`, `接地${i + 1}結果`, `接地${i + 1}改善對策`]).flat(),
    '檢修說明', '紀錄建立時間', '最後更新時間',
  ];
  ...
  function recordToCsvRow(record) {
    ...
    for (let i = 0; i < TABLE_ROWS; i++) {
      const r = (d.insulationRows || [])[i] || {};
      insulation.push(r.circuit || '', r.value || '', r.result || '', r.action || '');
    }
    const grounding = [];
    for (let i = 0; i < TABLE_ROWS; i++) {
