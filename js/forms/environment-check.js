(function () {
  const FORM_ID = 'environment-check';
  const FORM_TITLE = '其它環境設施檢查表';
  const FILE_PREFIX = '1-';

  const CHECK_ITEMS = [
    { no: '10.1', label: '10.1 通道有無阻礙' },
    { no: '10.2', label: '10.2 護欄是否穩固' },
    { no: '10.3', label: '10.3 設備周圍有無異物可以造成影響' },
    { no: '10.4', label: '10.4 爬梯是否穩固' },
    { no: '10.5', label: '10.5 屋頂或地面是否有樹根或雜草' },
    { no: '10.6', label: '10.6 屋頂排水孔是否有阻塞物' },
    { no: '10.7', label: '10.7 屋頂浪板、棚架是否生鏽' },
    { no: '10.8', label: '10.8 屋頂防水塗層是否破損修補' },
    { no: '10.9', label: '10.9 是否須修剪遮蔭樹木' },
    { no: '10.10', label: '10.10 是否更換滅火器' },
  ];
  ...
  els.bulkExportBtn.addEventListener('click', async (e) => {
    ...
    try {
      ...
      for (const id of ids) {
        ...
        if (!rec) continue;
        pages.push(EnvironmentCheckTemplate.buildPage1(rec.data, bg));
  ...
  const TEMPLATE_BG_PATH = '../pdf-templates/environment-check-p1.png';
  ...
  async function buildCurrentPdfBlob() {
    ...
    const data = gatherFormData();
    const bg = await loadBackground();
    const page = EnvironmentCheckTemplate.buildPage1(data, bg);
