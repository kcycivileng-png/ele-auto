(function () {
  const FORM_ID = 'cleaning-check';
  const FORM_TITLE = '清洗系統檢查表';
  const FILE_PREFIX = '1-';

  const CHECK_ITEMS = [
    { no: '7.1', label: '7.1 檢查各開關是否在正確位置' },
    { no: '7.2', label: '7.2 檢查各閥門功能是否正常' },
    { no: '7.3', label: '7.3 檢查有無漏水情況' },
    { no: '7.4', label: '7.4 檢查接合管路是否有鬆脫或損毀' },
  ];
  ...
  els.bulkExportBtn.addEventListener('click', async (e) => {
    ...
    try {
      ...
      for (const id of ids) {
        ...
        if (!rec) continue;
        pages.push(CleaningCheckTemplate.buildPage1(rec.data, bg));
  ...
  function validateForExport() {
    ...
    if (photoCtrls.cleaningEquip.getPhotos().length === 0) missing.push('「檢查項目-清洗設備」尚未上傳照片');
    return missing;
  }
  ...
  const TEMPLATE_BG_PATH = '../pdf-templates/cleaning-check-p1.png';
  ...
  async function buildCurrentPdfBlob() {
    ...
    const data = gatherFormData();
    const bg = await loadBackground();
    const page = CleaningCheckTemplate.buildPage1(data, bg);
