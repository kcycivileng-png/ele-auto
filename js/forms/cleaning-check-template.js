// 清洗系統檢查表 —— PDF「底圖套版」座標定義。
// 座標直接從原始 PDF（03_維運報告_套版範本.docx 匯出，對應國泰電業維運契約附件一 P34）量出來的（單位：pt，A4頁面）。
// 底圖 pdf-templates/cleaning-check-p1.png 已預先去除「電廠名稱/檢查日期/檢查人員/檢修說明」
// 這幾個一定會被覆蓋掉的欄位文字，其餘（框線、灰底、標準說明文字、頁尾）都是原始文件截圖。
// 單頁表單：檢查表格＋相片框格都在同一頁（原始文件本來就是這樣排版，不像太陽能模組那幾張表另外分頁）。
const CleaningCheckTemplate = (() => {
  const PAGE_W = 595.32;
  const PAGE_H = 841.92;
  ...
  const checkboxCols = { ok: 420, bad: 450.1, fixed: 485.5 };
  const CHECK_SIZE = 11;
  const ITEM_ROW_Y = [197.4, 213.5, 229.6, 245.6];
  const REPAIR_NOTE = { x: 59.8, y: 307, w: 480 };

  const PHOTOS = {
    cleaningEquip: [
      { x: 53.6, y: 372.6, w: 234.2, h: 115.0 },
      { x: 287.8, y: 372.6, w: 254.2, h: 115.0 },
    ],
    abnormal1: [{ x: 53.6, y: 503.7, w: 234.2, h: 114.5 }],
    abnormal2: [{ x: 287.8, y: 503.7, w: 254.2, h: 114.5 }],
  };

  const ABN_LABEL = {
    abnormal1: { x: 203.2, y: 489.8 },
    abnormal2: { x: 448.2, y: 489.8 },
  };
