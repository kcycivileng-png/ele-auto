(function () {
  const FORM_ID = 'inverter-check';
  const FORM_TITLE = '逆變器檢查表';
  const FILE_PREFIX = '1-';
  const GROUND_ROWS = 15; // 比照原始Word表格，固定15列

  // 注意：4.9機殼接地量測(年)不在這份清單裡（它由接地電阻量測表自動判定），
  // 匯出PDF時會在index=8的位置插回去，順序才會跟Word原始表格一致（4.1~4.8, 4.9, 4.10~4.12）。
  const CHECK_ITEMS = [
    { no: '4.1', label: '4.1 本體是否鏽蝕、破損、變形' },
    { no: '4.2', label: '4.2 固定架台是否鏽蝕、破損、變形' },
    { no: '4.3', label: '4.3 是否有異音或異味' },
    { no: '4.4', label: '4.4 螢幕、燈號是否正常' },
    { no: '4.5', label: '4.5 操控介面是否正常' },
    { no: '4.6', label: '4.6 散熱風扇及濾網清潔' },
    { no: '4.7', label: '4.7 散熱器上方是否保持清潔及空氣流通' },
    { no: '4.8', label: '4.8 檢查導線接頭是否有鬆脫、熱熔現象' },
    { no: '4.10', label: '4.10 檢查各電器是否有燒焦痕跡或異常' },
    { no: '4.11', label: '4.11 外部纜線是否損傷及接點是否鬆脫' },
    { no: '4.12', label: '4.12 周圍是否清潔' },
  ];

  const PHOTO_GROUPS = [
    { id: 'invPhoto1', title: '檢查項目-INVERTER清潔後外觀 設備1', single: true },
    { id: 'invPhoto2', title: '檢查項目-INVERTER清潔後外觀 設備2 (選填)', single: true },
    { id: 'invPhoto3', title: '檢查項目-INVERTER清潔後外觀 設備3 (選填)', single: true },
  ...
  PHOTO_GROUPS.forEach((g) => {
    const wrap = GROUND_PHOTO_IDS.includes(g.id) ? groundPhotoWrap : photoWrap;
  ...
  const groundingTable = KtFormKit.createEditableTable(document.getElementById('groundingTableWrap'), {
    ...
    onChange: scheduleAutosave,
    columns: [
      { key: 'boxNo', label: '逆變器編號', type: 'text', placeholder: '1' },
  ...
  // ======================================================================
  // 紀錄列表「多選模式」：一鍵匯出（合併成一份多頁PDF，每筆紀錄本身就是3頁）／一鍵刪除
  // ======================================================================
  ...
  els.bulkExportBtn.addEventListener('click', async (e) => {
    ...
    try {
      ...
      for (const id of ids) {
        ...
        pages.push(T.buildPageGrounding(rec.data, bg.p3));
  ...
  const TEMPLATE_BG_PATHS = {
    p1: '../pdf-templates/inverter-check-p1.png',
    p2: '../pdf-templates/inverter-check-p2-photos.png',
    p3: '../pdf-templates/inverter-check-p3-grounding.png',
  };
  ...
  async function buildCurrentPdfBlob() {
    ...
    const pages = [
      ...
      T.buildPageGrounding(data, bg.p3),
    ];
    const blob = await KtPdf.renderTemplatedPdf(pages);
