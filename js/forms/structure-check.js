(function () {
  const FORM_ID = 'structure-check';
  const FORM_TITLE = '結構系統檢查表';
  const FILE_PREFIX = '1-';

  const CHECK_ITEMS = [
    { no: '11.1', label: '11.1 是否破損、撞擊、變形或刮傷痕跡之不良情形' },
    { no: '11.2', label: '11.2 模組固定壓片是否鎖固，劃記無位移' },
    { no: '11.3', label: '11.3 鋁支架腳座是否鎖固且防水未破壞' },
    { no: '11.4', label: '11.4 各立柱與腳座結合螺絲組是否緊密結合無鬆動' },
    { no: '11.5', label: '11.5 支架現況水平與垂直是否歪斜' },
    { no: '11.6', label: '11.6 支架安裝橫桿截點是否牢固無下陷' },
    { no: '11.7', label: '11.7 支架各結合螺絲與螺帽是否鎖固且標記無位移' },
    { no: '11.8', label: '11.8 支架各結合部位螺絲、螺帽、華司及彈簧華司無漏缺' },
    { no: '11.9', label: '11.9 支架各部位是否有擠壓變形或破損情形' },
    { no: '11.10', label: '11.10 支架鎖固部位是否有變形或滑牙情形' },
    { no: '11.11', label: '11.11 支架結合鎖固後是否會晃動' },
    { no: '11.12', label: '11.12 支架北面架高擋風板是否受損' },
    { no: '11.13', label: '11.13 支架北面架高洩風口是否有阻礙物' },
    { no: '11.14', label: '11.14 線槽與蓋板是否鬆動' },
    { no: '11.15', label: '11.15 各迴路線路是否懸空並固定' },
  ];
  ...
  els.bulkExportBtn.addEventListener('click', async (e) => {
    ...
    try {
      ...
      for (const id of ids) {
        ...
        if (!rec) continue;
        pages.push(StructureCheckTemplate.buildPage1(rec.data, bg));
  ...
  const TEMPLATE_BG_PATH = '../pdf-templates/structure-check-p1.png';
  ...
  async function buildCurrentPdfBlob() {
    ...
    const data = gatherFormData();
    const bg = await loadBackground();
    const page = StructureCheckTemplate.buildPage1(data, bg);
